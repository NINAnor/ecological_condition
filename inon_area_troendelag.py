...
def run_script(iface):
    ...
    output_directory = '/inon_outputs_test3/'
    ...
    def calculate_area_over_clip(input_layer):
        ...
        for featc in input_layer.getFeatures():
            ...
            if geom.isMultipart() is False:
                print('geometry is not multipart')
                polyg = geom.asPolygon()
                if len(polyg) > 0:
                    area = area + calculator.measurePolygon(polyg[0])
                    print('single part area is ' + str(area))

            # is Multipart
            else:
                print('geometry is multipart')
                multi = geom.asMultiPolygon()
                for polyg in multi:
                    area = area + calculator.measurePolygon(polyg[0])
                    print('multipart area is ' + str(area))
