...
def run_script(iface):
    ...
    output_directory = '/inon_outputs_final/'
    ...
    for file in os.listdir(wilderness_areas):
        ...
        project.addMapLayer(inon_ar50_input_layer)
    ...
    for file in os.listdir(ar5_directory):
        ...
        for file in os.listdir(wilderness_areas):
            ...
            if filename.endswith('.gpkg'):
                ...
                inon_layer_kommune = geoprocess_layer(clip_alg, inon_layer, n50_kommune,
                                                              ar5_kommune_name)
                ...
                ar50_coverunregistered_name = 'ar50_coverunregistered_' + kommune_name
                ...
                year_areas[year]['total_area'].append(total_area)

                myr_area = calculate_area_over_clip(ar5_myr_kommune)
                year_areas[year]['myr_area'].append(myr_area)
                ...
                year_areas[year]['myr_perc'].append(myr_perc)
                year_areas[year]['grass_perc'].append(grass_perc)
                year_areas[year]['mountain_perc'].append(mountain_perc)
                year_areas[year]['forest_11_perc'].append(forest_11_perc)
                year_areas[year]['forest_13_perc'].append(forest_13_perc)
                year_areas[year]['forest_14_perc'].append(forest_14_perc)
                year_areas[year]['forest_ar50_perc'].append(forest_ar50_perc)
                year_areas[year]['myr_ar50_perc'].append(myr_ar50_perc)
                year_areas[year]['mountain_ar50_perc'].append(mountain_ar50_perc)
    ...
    for key, value in inon_layers.items():
        with edit(value):
            for i, feat in enumerate(value.getFeatures()):
                feat.setAttribute('total_area', sum(year_areas[key]['total_area']))
                feat.setAttribute('myr_area', sum(year_areas[key]['myr_area']))
                feat.setAttribute('grass_area', sum(year_areas[key]['grass_area']))
                feat.setAttribute('mountain_area', sum(year_areas[key]['mountain_area']))
                feat.setAttribute('forest_11_area', sum(year_areas[key]['forest_11_area']))
                feat.setAttribute('forest_13_area', sum(year_areas[key]['forest_13_area']))
                feat.setAttribute('forest_14_area', sum(year_areas[key]['forest_14_area']))
                feat.setAttribute('forest_ar50_area', sum(year_areas[key]['forest_ar50_area']))
                feat.setAttribute('myr_ar50_area', sum(year_areas[key]['myr_ar50_area']))
                feat.setAttribute('mountain_ar50_area', sum(year_areas[key]['mountain_ar50_area']))

                feat.setAttribute('myr_area_perc', sum(year_areas[key]['myr_area_perc']))
                feat.setAttribute('grass_area_perc', sum(year_areas[key]['grass_area_perc']))
                feat.setAttribute('mountain_area_perc', sum(year_areas[key]['mountain_area_perc']))
                feat.setAttribute('forest_11_area_perc', sum(year_areas[key]['forest_11_area_perc']))
                feat.setAttribute('forest_13_area_perc', sum(year_areas[key]['forest_13_area_perc']))
                feat.setAttribute('forest_14_area_perc', sum(year_areas[key]['forest_14_area_perc']))
                feat.setAttribute('forest_ar50_perc', sum(year_areas[key]['forest_ar50_perc']))
                feat.setAttribute('myr_ar50_perc', sum(year_areas[key]['myr_ar50_perc']))
                feat.setAttribute('mountain_ar50_perc', sum(year_areas[key]['mountain_ar50_perc']))
